...
function sidebarTarget(location) {
  const sidebarMap = [
    ['главная', { x: 53, y: 70 }],
    ['складские операции', { x: 53, y: 102 }],
    ['справочник', { x: 53, y: 132 }],
    ['финансовые операции', { x: 53, y: 163 }],
    ['отчёты', { x: 53, y: 194 }],
    ['отчеты', { x: 53, y: 194 }],
    ['настройки', { x: 53, y: 225 }],
    ['кабинет', { x: 53, y: 256 }],
    ['выход', { x: 53, y: 450 }],
  ];
...
function targetForProducts(location, lessonName) {
  ...
  if (containsAny(t, ['поле поиска', 'поиск'])) return { x: 512, y: 175 };
  if (containsAny(t, ['фильтры', 'фильтр'])) {
    if (containsAny(t, ['категория'])) return { x: 640, y: 175 };
    if (containsAny(t, ['склад'])) return { x: 683, y: 175 };
    return { x: 683, y: 175 };
  }
  if (containsAny(t, ['иконка карандаша', 'редактировать'])) return { x: 790, y: 168 };
  if (containsAny(t, ['иконка корзины', 'удалить'])) return { x: 820, y: 168 };
  if (containsAny(t, ['иконка звезды', 'избран'])) return { x: 760, y: 168 };
  if (containsAny(t, ['клик на название', 'карточка'])) return { x: 320, y: 168 };
...
function targetForDashboard(location, lessonName) {
  ...
  if (containsAny(t, ['главная'])) return { x: 53, y: 70 };
  if (containsAny(t, ['кабинет'])) return { x: 53, y: 256 };
  ...
  if (containsAny(t, ['отчет о продаже'])) return { x: 780, y: 79 };
  if (containsAny(t, ['возврат прихода'])) return { x: 502, y: 211 };
  if (containsAny(t, ['трансфер'])) return { x: 632, y: 211 };
  if (containsAny(t, ['реализация'])) return { x: 780, y: 211 };
  if (containsAny(t, ['переоценка'])) return { x: 240, y: 326 };
...
function targetForBalance(location, lessonName) {
  const t = `${location} ${lessonName}`;
  if (containsAny(t, ['баланс'])) return { x: 630, y: 16 };
  if (containsAny(t, ['пополнение'])) return { x: 680, y: 16 };
  if (containsAny(t, ['дата', 'период'])) return { x: 284, y: 45 };
  if (containsAny(t, ['сформировать'])) return { x: 495, y: 45 };
  return { x: 630, y: 16 };
}
...
function targetForListPage(location, lessonName) {
  ...
  if (containsAny(t, ['фильтры', 'фильтр'])) {
    if (containsAny(t, ['дата', 'период'])) return { x: 300, y: 54 };
    if (containsAny(t, ['поставщик'])) return { x: 450, y: 54 };
    if (containsAny(t, ['статус'])) return { x: 520, y: 54 };
    if (containsAny(t, ['касса'])) return { x: 590, y: 54 };
    return { x: 450, y: 54 };
  }
...
// Generate GitHub Release URL for a video
...
async function main() {
  ...
  for (const lesson of lessons) {
    ...
    for (const locale of LOCALES) {
      ...
      if (!localReady || FORCE_REGENERATE) {
      ...
      // Store GitHub Release URL in database
      const url = getGitHubReleaseUrl(lesson.order, locale, lesson);
      map[locale] = url;
      
      console.log(`Lesson ${lesson.order} ${locale}: video ready -> ${url}`);
    }
    
    // Update database with GitHub Release URLs
  ...
  console.log('\nNext step: Create GitHub Release and upload videos');
  console.log('Run: gh release create lesson-videos --repo FreedoomForm/genspark public/generated-videos/*.mp4');
}

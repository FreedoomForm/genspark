...
async function login(page) {
  ...
  await page.waitForURL(/\/dashboard|\/(ado|avatars|barcodes|cabinet|warehouse|reports|settings|salepoint|admin)/, { timeout: 120000 }).catch(() => {});
  await page.waitForTimeout(LOGIN_WAIT_MS);
...
async function fetchRouteAliases(page) {
  ...
  // Note: /vendor routes removed - only /salepoint, /admin, /dashboard prefixes are used
...
async function openLesson(page, lesson, aliases) {
  ...
  for (const candidate of candidates) {
    ...
    if (!looksLikeDashboard(currentUrl, bodyText) || /\/dashboard\//.test(currentUrl) || /\/admin\//.test(currentUrl) || /\/salepoint\//.test(currentUrl)) {
